...
#if defined(NDEBUG)
# error "Binarycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa226534c221330b43afefb2c572cd0e71770655151218d33c9eeec27f5a83e89");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Binarycoin: starting difficulty is 1 / 2^12
...
{
    // Binarycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Binarycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Binarycoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Binarycoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Binarycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Binarycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd8e18e75641e4c3619ff0cb78cfe134bc02390df87e996d9ff201fcc3c86045d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Germany, 10/Sep/2018 Michael Dick, started a new life section at Bosch";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a42ea0b23464e9c56e219cc044b612ecd533a0e27525273d6842e71c57fa9fb7924bdd84830d1c0849c5b53d75e423c04dccc1d1b18d127449f21347dbce9e94") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1537720912;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085261134;

        if (fTestNet)
        {
            block.nTime    = 1537720881;
            block.nNonce   = 385466415;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x8e23853eac90109656c67f9e4775323e6856bab42c9863947078851b4031f864"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf0, 0xc2, 0xb0, 0xd5 }; // Binarycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BinarycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BinarycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BinarycoinMiner(CWallet *pwallet)
{
    printf("BinarycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("binarycoin-miner");
    ...
    try { loop {
        ...
        printf("Running BinarycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BinarycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BinarycoinMiner, pwallet));
}
